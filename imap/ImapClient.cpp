...
ssize_t curl_callback(char* buf, size_t size, size_t nmemb, void* up) {
...
bool ImapClient::deleteMail(ImapMail mail) {
    ...
    if(this->curl) {
        string url = this->url+mail.folder;
        curl_easy_setopt(curl, CURLOPT_URL, url.c_str());
        curl_easy_setopt(curl, CURLOPT_CUSTOMREQUEST, string("STORE "+to_string(mail.id)+" +Flags \\Deleted").c_str());
        res = curl_easy_perform(curl);
        if(res != CURLE_OK) {
            cerr << "curl_easy_perform() failed: " << curl_easy_strerror(res) << endl;
            return false;
